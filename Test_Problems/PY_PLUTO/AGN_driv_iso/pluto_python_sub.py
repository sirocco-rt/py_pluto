...
def pluto_input_file(tlim,dt,data):
    output=open('pluto.ini','w')
    output.write("[Grid]\n")
    output.write("\n")
    output.write("X1-grid 1 "+str(data["R_MIN"])+" "+str(data["N_R"])+" r "+str(data["R_MAX"])+"  "+str(data["s_r"])+"\n")
    output.write("X2-grid 1 "+str(data["T_MIN"])+" "+str(data["N_T"])+" r "+str(data["T_MAX"])+"  "+str(data["s_theta"])+"\n")
    ...
    output.write("dbl        "+str(dt)+"  -1    single_file\n")
...
def pluto2py_rtheta(ifile):
    ...
    light_speed = np.ones(np.shape(python_density))*1.7e10 ### So that sqrt(vx1**2+vx2**2) is smaller than c
    python_vz = np.minimum(python_vz,light_speed)
    python_vz = np.maximum(python_vz,-light_speed)
    python_vx = np.minimum(python_vx,light_speed)
    python_vx = np.maximum(python_vx,-light_speed)
    python_vy = np.minimum(python_vy,light_speed)
    python_vy = np.maximum(python_vy,-light_speed)
