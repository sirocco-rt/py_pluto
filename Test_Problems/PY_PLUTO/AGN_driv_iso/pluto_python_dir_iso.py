...
data["f_uv"]=1.0
data["f_x"]=0.0 #This is the proportion of disk flux we assume comes from the middle as X-rays
...
data["boundary_layer"]="no" #turns on the boundary layer
...
if t0==0.0:
    ...
    t0=2.e3
init_py_cycles=20 #This is the initial number of cycles that python will do - worth doing a few more here than we will in future to get a bit of convergence in ionization state 
...
subprocess.check_call("cp directional_flux_theta_opt_thin.dat directional_flux_theta.dat",shell=True)   
subprocess.check_call("cp directional_flux_phi_opt_thin.dat directional_flux_phi.dat",shell=True)   
subprocess.check_call("cp directional_flux_r_opt_thin.dat directional_flux_r.dat",shell=True)   




for i in range(istart,10000): #Do a loop from istart to some big number. You can set the big number to anything - but if something goes wrong, it should be limited to avoid spamming loads of files that could jam things up..
    ...
    pps.pluto_input_file(time,dt,data) #This makes a pluto.ini file then the next few lines run pluto - either restarting, or from scratch and using mpi if necessary. The lines construct an argument to be sent to subprocess which actually runs the command
...
#The next chunk deals with running python, 
    ...
    if i>0: #If we are restarting
        ...
        logfile.write("command line: "+cmdline+"\n")          
        subprocess.check_call(cmdline,shell=True) 
        #now we run python -- turned to special relativity compared to CV where we were using the -classic flag 
        cmdline="mpirun -n "+str(data["nproc_py"])+" py"+data["python_ver"]+" -f -r --rseed input.pf > python_log"        
    else:   
        cmdline="mpirun -n "+str(data["nproc_py"])+" py"+data["python_ver"]+" -f --rseed input.pf > python_log"
