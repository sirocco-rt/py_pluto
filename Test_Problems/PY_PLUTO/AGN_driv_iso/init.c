...
//      
  ...
  if (v[RHO]<1e-24)  //Set a lower density bound
  {
  	v[RHO]=1e-24; 
...
if (side == 0) {    /* -- check solution inside domain -- */
	...
	{	
		...
		if (d->Vc[RHO][k][j][i]*UNIT_DENSITY < 1.e-24) //Set a lower density throughout the domain
		{       
			d->Vc[RHO][k][j][i] = 1.e-24/UNIT_DENSITY;
			//M_max_global[k][j][i] = 0.;
			convert_to_cons = 1;
		}
		if (convert_to_cons){
		...
		if (d->flag[k][j][i] & FLAG_NEGATIVE_DENSITY) {
			M_max_global[k][j][i] = M_min;
			M_max_global[k][j][i+1] = M_min;
			M_max_global[k][j][i-1] = M_min;
			M_max_global[k][j+1][i] = M_min;
			M_max_global[k][j-1][i] = M_min;
			M_max_global[k][j-1][i-1] = M_min;
			M_max_global[k][j+1][i+1] = M_min;
			M_max_global[k][j-1][i+1] = M_min;
			M_max_global[k][j+1][i-1] = M_min;
		//	printf("Change M_max_global\n");
		} else {
			M_max_global[k][j][i] = M_max;
		}

		if (j==grid->np_int[JDIR]+2 && 	(fabs(x2_glob[grid->np_int_glob[JDIR]+2]-x2[j])/x2_glob[grid->np_int_glob[JDIR]+2]) < 1e-20)  //This should be the last 'real' theta bin - before the ghost zones.
		{
			r = x1[i]*UNIT_LENGTH;
			theta= x2[j];
			...
			d->Vc[VX1][k][j][i]=0.0;									//Set radial velocity at the midplane to zero
                        ...
                        d->Vc[VX2][k][j][i]=0.; //conserve momentum
...
{
     if (d->Vc[VX1][k][j][i] >= 0.){
	d->Vc[RHO][k][j][i]=d->Vc[RHO][k][j][IBEG];      //apply normal BCs
	d->Vc[VX1][k][j][i]=0.;      //apply normal BCs
	d->Vc[VX2][k][j][i]=d->Vc[VX2][k][j][IBEG];      //apply normal BCs
	d->Vc[VX3][k][j][i]=d->Vc[VX3][k][j][IBEG];
    } else {
	d->Vc[RHO][k][j][i]=d->Vc[RHO][k][j][IBEG];      //apply normal BCs
	d->Vc[VX1][k][j][i]=d->Vc[VX1][k][j][IBEG];      //apply normal BCs
	d->Vc[VX2][k][j][i]=d->Vc[VX2][k][j][IBEG];      //apply normal BCs
	d->Vc[VX3][k][j][i]=d->Vc[VX3][k][j][IBEG];	
    }	
...
{
    if (d->Vc[VX1][k][j][i] >= 0.){
	d->Vc[RHO][k][j][i]=d->Vc[RHO][k][j][IEND];      //apply normal BCs
	d->Vc[VX1][k][j][i]=d->Vc[VX1][k][j][IEND];      //apply normal BCs
	d->Vc[VX2][k][j][i]=d->Vc[VX2][k][j][IEND];      //apply normal BCs
...
//if (box->vpos == CENTER) {
	...
	if (d->Vc[VX2][k][j][i] >= 0.){
        	d->Vc[RHO][k][j][i] = d->Vc[RHO][k][JBEG][i];      //apply normal BCs
        	d->Vc[VX1][k][j][i] = d->Vc[VX1][k][JBEG][i];      //apply normal BCs
        	d->Vc[VX2][k][j][i] = -d->Vc[VX2][k][JBEG][i];      //apply normal BCs
       		d->Vc[VX3][k][j][i] = -d->Vc[VX3][k][JBEG][i];
    	} else {
        	d->Vc[RHO][k][j][i] = d->Vc[RHO][k][JBEG][i];      //apply normal BCs
        	d->Vc[VX2][k][j][i] = 0.;      //apply normal BCs  
        	d->Vc[VX1][k][j][i] = 0.;      //apply normal BCs
        	d->Vc[VX3][k][j][i] = -d->Vc[VX3][k][JBEG][i];
    	}
        //M_max_global[k][j][i] = 0.;
...
//if (box->vpos == CENTER) {
	BOX_LOOP(box,k,j,i){  
    	...
    	if (d->Vc[VX2][k][j][i] >= 0.){
        	d->Vc[RHO][k][j][i]=d->Vc[RHO][k][JEND][i];      //apply normal BCs
        	d->Vc[VX1][k][j][i]=d->Vc[VX1][k][JEND][i];      //apply normal BCs
        	d->Vc[VX2][k][j][i]=d->Vc[VX2][k][JEND][i];      //apply normal BCs
        	d->Vc[VX3][k][j][i]=d->Vc[VX3][k][JEND][i];
    	} else {
        	d->Vc[RHO][k][j][i]=d->Vc[RHO][k][JEND][i];      //apply normal BCs
        	d->Vc[VX2][k][j][i]=0.;      //apply normal BCs  
        	d->Vc[VX1][k][j][i]=d->Vc[VX1][k][JEND][i];      //apply normal BCs
...
double x11[2],x22[2],flux_t,flux_r;
...
double ds,maxds,temp,dtan_theta,r_pos;
...
for (iangle=0;iangle<NFLUX_ANGLES;iangle++)
{
    DOM_LOOP(k,j,i) //Now compute dvdr for use in line driving calculations
    {
	    ...
	    x11_interp[0][k][j][i]=x1l[i]*UNIT_LENGTH;
	    x11_interp[1][k][j][i]=x2l[j];
	
	    x22_interp[0][k][j][i]=x1l[i+1]*UNIT_LENGTH;
	    x22_interp[1][k][j][i]=x2l[j+1];
	    ...
	    maxds/=2.; //This should mean than the distance we move to compute dv/ds is within the interpolation grid
	
	    //We now need the direction to move for the disk flux - that depends on the flux - we first work out the modulus of  the flux
	    mod_flux=sqrt(pow(flux_t_UV[iangle][k][j][i],2)+pow(flux_p_UV[iangle][k][j][i],2)+pow(flux_r_UV[iangle][k][j][i],2));
	    ...
	    else
	    {
	    	...
	    	x=x1[i]*sin(x2l[j])*UNIT_LENGTH; //Get the cell centre in cartesian coords
	    	z=x1[i]*cos(x2l[j])*UNIT_LENGTH;
	    
	    	theta_angle = (iangle+0.5)*6.283185307179586/36.;
...
for (iangle=0;iangle<NFLUX_ANGLES;iangle++)
{
	...
	{		
    	...
    	vx1=(ans1[0]*UNIT_VELOCITY*sin(x2l[j])+ans1[1]*UNIT_VELOCITY*cos(x2l[j]));
    	vz1=(ans1[0]*UNIT_VELOCITY*cos(x2l[j])-ans1[1]*UNIT_VELOCITY*sin(x2l[j]));
    	...
    	else
    	{
			...
			if (ans2[0]!=ans2[0]){
				printf("ans2[0] %e x11[0] %e x11[1] %e x22[0] %e x22[1] %e v11[0] %e v11[1] %e v12[0] %e v12[1] %e v21[0] %e v21[1] %e v22[0] %e v22[1] %e loc[0] %e loc[1] %e\n",ans2[0],x11[0],x11[1],x22[0],x22[1],v11[0],v11[1],v12[0],v12[1],v21[0],v21[1],v22[0],v22[1],loc[0],loc[1]);
			}
 ...
 *********************************************************************** */
{
    ...
    for (iangle=0;iangle<NFLUX_ANGLES;iangle++)
    {
        ...
        if (dvds_array[iangle][k][j][i]>0.0)
        {
            ...
            if (M_UV>M_max_global[k][j][i]){ 
//		        printf("M (%e) > M_max_global (%e) at i=%i, j=%i\n",M_UV,M_max_global[k][j][i],i,j);
		        M_UV=M_max_global[k][j][i];
	        }
        }
        else {
            //M_UV=M_max; //This is a bit of a fudge - the only way this should be negative is if there is no flux..
            M_UV=0.; //This is a bit of a fudge - the only way this should be negative is if there is no flux..
            //printf("At i=%i, j=%i, iangle=%i, we have dvds_array=%e\n",i,j,iangle,dvds_array[iangle][k][j][i]);
	    }
        ...
        if (g_stepNumber>1)
        {
            ...
            //g[JDIR]=0.;
            //				if (i==2 && j==2)
            //					printf ("Finished doing angle %i i  %i j %i flux %e  dvds %e gr %e gt %e\n",iangle,i,j,flux_r_UV[iangle][k][j][i],dvds_array[iangle][k][j][i],g[IDIR],g[JDIR]);
        }
        else
        {
        ...
        g_y=g_rad[1][k][j][i];
        g_z=g_rad[2][k][j][i];
        //g[IDIR]=g[IDIR] + g_x*sin(theta)+g_z*cos(theta);
        //g[JDIR]=g[JDIR] + g_x*cos(theta)-g_z*sin(theta);
        //g[KDIR]=g[KDIR] + g_y;
        //g[IDIR]=0.;
        //g[JDIR]=0.;
        //g[KDIR]=0.;
